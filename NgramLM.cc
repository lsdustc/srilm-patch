...
Ngram::getpruneProbs(unsigned size,unsigned minorder){
    ...
    long long ngram_counts = 0;
    while(it != perpChanges.end())
    {
        //it->first;
        //it->second;
        ngram_counts += it->second;
        if(ngram_counts>prune_size)
        {
            threshold = it->first * eplison;
            cerr << "Find best threshold "<<threshold << endl;
            cerr << "Pruned model size "<<ngram_counts +reserve_size << endl;
            break;
        }
        it ++;         
    }
